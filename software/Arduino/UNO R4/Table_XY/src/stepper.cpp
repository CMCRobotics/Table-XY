...
/*
  Original for AVR = 16 MHz
  #define AMASS_LEVEL1 (F_CPU/8000) // Over-drives ISR (x2). Defined as F_CPU/(Cutoff frequency in Hz)
  #define AMASS_LEVEL2 (F_CPU/4000) // Over-drives ISR (x4)
  #define AMASS_LEVEL3 (F_CPU/2000) // Over-drives ISR (x8)
*/

// For R4: 48MHz = 16MHz * 3
#define AMASS_LEVEL1 ((F_CPU * 3)/(8000)) // Over-drives ISR (x2). Defined as F_CPU/(Cutoff frequency in Hz)
#define AMASS_LEVEL2 ((F_CPU * 3)/(4000)) // Over-drives ISR (x4)
#define AMASS_LEVEL3 ((F_CPU * 3)/(2000)) // Over-drives ISR (x8)
...
// When the Step Pulses must be stopped, action should be done after clearing the Pulse signal.
static volatile boolean step_pulse_stop;
...
void st_wake_up()
{
  ...
  if (bit_istrue(settings.flags,BITFLAG_INVERT_ST_ENABLE)) { STEPPERS_DISABLE_SET; }
  else { STEPPERS_DISABLE_CLR; }
  ...
  if (sys.state & (STATE_CYCLE | STATE_HOMING)){
    ...
    // This part defines the time where the Step Pulse must be cleared.
    // It defines CMPB interrupt time

    #ifdef STEP_PULSE_DELAY
      // CMPB must arrive after the Delay CMPA, therefore add the Delay
      st.step_pulse_time = (u_int32_t) (settings.pulse_microseconds + STEP_PULSE_DELAY) * MOTOR_TICK_PER_USEC;
      ...
      // Set CMPB for generating the Step pulse width: clear the Step pulse(s) at that time
      st.step_pulse_time = (u_int32_t) (settings.pulse_microseconds * MOTOR_TICK_PER_USEC);
    #endif

    MOTOR_STEP_PULSE_WIDTH_REGISTER_VALUE = st.step_pulse_time;

    // Enable Stepper Driver Interrupt
    step_pulse_stop = false;
    GPT320_enable();
  }
}
...
void st_go_idle()
{
  ...
  // This MUST be done in IRS CMPB function, for allowing the Step pulse to be ended correctly
  step_pulse_stop = true;
  ...
  if (pin_state) { STEPPERS_DISABLE_SET; }
  else { STEPPERS_DISABLE_CLR; }
}
...
void ISRinterrupt_Motor_Main(void){
  // This is only for allowing to correctly generate the Test Point,
  //  without having to look at all the return positions

  //-------------------------------------------------------------------------
  // Also modify at the end of function & at return, if it is Set/Clear
  TEST_3_SET; // Debug: Used to time ISR
  //-------------------------------------------------------------------------

  functionISRinterrupt_Motor_Main();

  //-------------------------------------------------------------------------
  TEST_3_CLR; // Debug: Used to time ISR
  //-------------------------------------------------------------------------
...
void functionISRinterrupt_Motor_Main(void)
{
  //Clear Interrupt flag
  R_GPT0->GTST_b.TCFPO = 0;                // Clear Overflow Flag
  R_ICU->IELSR_b[GPT320_INTERRUPT_EVENT_OVF_IELSRindex].IR = 0;   // Clear Interruption Flag
  ...
  DIRECTION_PORT_WRITE = (DIRECTION_PORT_READ & ~DIRECTION_MASK) | (st.dir_outbits & DIRECTION_MASK);
  __NOP();   __NOP();   __NOP();

   // Then pulse the stepping pins
  #ifdef STEP_PULSE_DELAY
    // Original, but creates problems with TestPoint
    //st.step_bits = (STEP_PORT_READ & ~STEP_MASK) | st.step_outbits; // Store out_bits to prevent overwriting.

    st.step_bits = st.step_outbits; // Store out_bits to prevent overwriting.

  #else  // Normal operation
    STEP_PORT_WRITE = (STEP_PORT_READ & ~STEP_MASK) | st.step_outbits;
  #endif
  ...
  if (st.exec_segment == NULL) {
    ...
    if (segment_buffer_head != segment_buffer_tail) {
      ...
      st.exec_segment = &segment_buffer[segment_buffer_tail];
        ...
        // When AMASS is disabled, set timer prescaler for segments with slow step frequencies (< 250Hz).
      ...
      R_GPT0->GTCR_b.CST = 0;       // GPT Stopped
      __NOP();__NOP();__NOP();

      R_GPT0->GTPR_b.GTPR = st.exec_segment->cycles_per_tick;  // Max counter for setting the Step period
      R_GPT0->GTPBR_b.GTPBR = st.exec_segment->cycles_per_tick;

      R_GPT0->GTCR_b.CST = 1;       // GPT Started
...
/* The Stepper Port Reset Interrupt: Timer320 CMPB interrupt handles the falling edge of the step
   pulse. This should always trigger before the next Timer320 OVF interrupt and ALWAYS
   finish, if Timer is disabled after completing a move.
   NOTE: Interrupt collisions between the serial and stepper interrupts can cause delays by
   a few microseconds, if they execute right before one another. Not a big deal, but can
   cause issues at high step rates if another high frequency asynchronous interrupt is
   added to Grbl.
*/
// This ISR resets the motor port after a short period (settings.pulse_microseconds)
// completing one step cycle.


// Step Pulse ended
void ISRinterrupt_Motor_StepPulseEnd(void) {
  //-------------------------------------------------------------------------
  // Also modify at the end of function, if it is Set/Clear
  // TEST_1_SET; // Debug: Used to time ISR
  //-------------------------------------------------------------------------

  //Pulse high Ended, Clear Motor step Pulse
  R_GPT0->GTST_b.TCFB = 0;                // Clear Comp-B Flag
  R_ICU->IELSR_b[GPT320_INTERRUPT_EVENT_CMPB_IELSRindex].IR = 0;   // Clear Interruption Flag

  // Reset stepping pins (leave the direction pins)
  STEP_PORT_WRITE = (STEP_PORT_READ & ~STEP_MASK) | (step_port_invert_mask & STEP_MASK);

  // If No more Step pulse must be generated
  if(step_pulse_stop){
    step_pulse_stop = false;
    // Stop Timer GPT320
    GPT320_disable();
  ...
  //-------------------------------------------------------------------------
  // Also modify at the end of function
  // TEST_1_CLR; // Debug: Used to time ISR
  //-------------------------------------------------------------------------
  ...
  // initiated after the STEP_PULSE_DELAY time period has elapsed. The ISR TIMER320 CMPA interrupt
  // will then trigger after the appropriate settings.pulse_microseconds, as in normal operation.
  // The new timing between direction, step pulse, and step complete events are setup in the
  // st_wake_up() routine.


  // Delay between Direction signal and Step Pulse signal
void ISRinterrupt_Motor_DelayEnd(void) {
  //-------------------------------------------------------------------------
  // Also modify at the end of function, if it is Set/Clear
  // TEST_1_SET; // Debug: Used to time ISR
  //-------------------------------------------------------------------------

  //Delay Direction-Step ended, set Motor step Pulse
  R_GPT0->GTST_b.TCFA = 0;                // Clear Comp-A Flag
  R_ICU->IELSR_b[GPT320_INTERRUPT_EVENT_CMPA_IELSRindex].IR = 0;   // Clear Interruption Flag

  // Original, but creates problems with TestPoint
  // STEP_PORT_WRITE = st.step_bits; // Begin step pulse.

  STEP_PORT_WRITE = (STEP_PORT_READ & ~STEP_MASK) | st.step_bits;
...
void st_reset()
{
  ...
  STEP_PORT_WRITE = (STEP_PORT_READ & ~STEP_MASK) | step_port_invert_mask;
  DIRECTION_PORT_WRITE = (DIRECTION_PORT_READ & ~DIRECTION_MASK) | dir_port_invert_mask;
}
...
void stepper_init()
{
  ...
  // Configure Timer 320: Stepper Driver Interrupt
  GPT320_init();
...
void st_prep_buffer()
{
  ...
  while (segment_buffer_tail != segment_next_head) { // Check if we need to fill the buffer.
    ...
    */
    ...
    cycles = ceil( (MOTOR_TICK_PER_SEC * inv_rate )); // (cycles/step)
      ...
      // R3 Original: prep_segment->cycles_per_tick = cycles;
      prep_segment->cycles_per_tick = cycles * 0.2; // Period / 5

    #else
		// TODO if no AMASS is used
