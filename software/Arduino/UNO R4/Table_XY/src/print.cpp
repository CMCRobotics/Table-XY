...
void print_unsigned_int8(uint8_t n, uint8_t base, uint8_t digits)
{
  ...
  unsigned char buf_string[digits + 1];
  uint8_t i = 0;
  ...
  for (; i > 0; i--)       buf_string[digits - i] = '0' + buf[i - 1];

  buf_string[digits] = 0;   // Make a string
...
void print_uint32_base10(uint32_t n)
{
  ...
  for (; i > 0; i--)       buf_string[idx - i] = '0' + buf[i - 1];

  buf_string[idx] = 0;   // Make a string
  printString( (char *) buf_string);
...
void printFloat(float n, uint8_t decimal_places)
{
  ...
  for (; i > 0; i--)       buf_string[idx - i] = buf[i - 1];
