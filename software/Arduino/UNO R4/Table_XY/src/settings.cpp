...
{
  eeprom_put_char(0, SETTINGS_VERSION, false);
...
void settings_restore(uint8_t restore_flag) {
  ...
  if (restore_flag & SETTINGS_RESTORE_STARTUP_LINES) {
	#if N_STARTUP_LINE > 0
	eeprom_put_char(EEPROM_ADDR_STARTUP_BLOCK, 0, false);
	#endif
	#if N_STARTUP_LINE > 1
	eeprom_put_char(EEPROM_ADDR_STARTUP_BLOCK+(LINE_BUFFER_SIZE+1), 0, false);
	#endif
  }
...
uint8_t read_global_settings() {
  ...
  uint8_t version = eeprom_get_char(0, false);
